...
// DuckDBChangeStorage adapts the shared DuckDB change store to Azure provider types.
...
func NewDuckDBChangeStorage(dbPath string) (*DuckDBChangeStorage, error) {
	store, err := changestore.NewDuckDBStore(dbPath, "azure_change_tracking.db", "Azure")
	if err != nil {
		return nil, err
	}
