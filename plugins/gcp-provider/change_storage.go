...
func (dcs *DuckDBChangeStorage) initializeSchema() error {
	schemas := []string{
		...
		`CREATE TABLE IF NOT EXISTS change_analytics (
			id VARCHAR PRIMARY KEY,
			provider VARCHAR NOT NULL,
			date_bucket TIMESTAMP NOT NULL,
			bucket_type VARCHAR NOT NULL, -- 'hour', 'day', 'week', 'month'
			service VARCHAR,
		...
		`CREATE TABLE IF NOT EXISTS change_streams (
			id VARCHAR PRIMARY KEY,
			name VARCHAR NOT NULL,
			provider VARCHAR NOT NULL,
	...
	log.Printf("Change tracking database schema initialized successfully")
	return nil
}
...
func (dcs *DuckDBChangeStorage) StoreChanges(changes []*ChangeEvent) error {
	...
	// Update analytics for all changes
	go func() {
		for _, change := range changes {
			dcs.updateAnalytics(change)
		}
	}()
...
func (dcs *DuckDBChangeStorage) buildChangeQuery(query *ChangeQuery) (string, []interface{}) {
	...
	if len(query.Severities) > 0 {
		...
		for i, severity := range query.Severities {
			placeholders[i] = "?"
	...
	if query.ResourceFilter != nil {
		...
		if len(rf.Services) > 0 {
			placeholders := make([]string, len(rf.Services))
			for i, service := range rf.Services {
				placeholders[i] = "?"
		...
		if len(rf.Projects) > 0 {
			placeholders := make([]string, len(rf.Projects))
			for i, project := range rf.Projects {
				placeholders[i] = "?"
		...
		if len(rf.Regions) > 0 {
			placeholders := make([]string, len(rf.Regions))
			for i, region := range rf.Regions {
				placeholders[i] = "?"
...
func (dcs *DuckDBChangeStorage) updateAnalytics(change *ChangeEvent) {
	if change == nil {
		return
	}
	...
	if change.ImpactAssessment != nil {
		impactScore = change.ImpactAssessment.RiskScore
	}

	// Update hourly bucket
	dcs.updateAnalyticsBucket(change, "hour", impactScore)

	// Update daily bucket
	dcs.updateAnalyticsBucket(change, "day", impactScore)
}

func (dcs *DuckDBChangeStorage) updateAnalyticsBucket(change *ChangeEvent, bucketType string, impactScore float64) {
	var dateBucket time.Time

	switch bucketType {
	case "hour":
		dateBucket = change.Timestamp.Truncate(time.Hour)
	case "day":
		dateBucket = change.Timestamp.Truncate(24 * time.Hour)
	case "week":
		dateBucket = change.Timestamp.Truncate(7 * 24 * time.Hour)
	case "month":
		dateBucket = time.Date(change.Timestamp.Year(), change.Timestamp.Month(), 1, 0, 0, 0, 0, change.Timestamp.Location())
	default:
		return
	}

	// Generate unique ID for this bucket
	id := fmt.Sprintf("%s_%s_%s_%s_%s_%s_%s",
		change.Provider,
		dateBucket.Format("2006-01-02T15:04:05Z"),
		bucketType,
		change.Service,
		change.ResourceType,
		string(change.ChangeType),
	...
	query := `INSERT INTO change_analytics (
		id, provider, date_bucket, bucket_type, service, resource_type,
		change_type, severity, count, total_impact_score, avg_impact_score,
		created_at, updated_at
	) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?)
	ON CONFLICT (id) DO UPDATE SET
		count = count + 1,
		total_impact_score = total_impact_score + ?,
		avg_impact_score = total_impact_score / count,
		updated_at = ?`
	...
	if err != nil {
		log.Printf("Failed to update analytics bucket: %v", err)
	}
}