...
func discoverServices() ([]string, error) {
	...
	// 3. Discover from GitHub API when explicitly enabled. This keeps
	// auto-discovery deterministic and offline by default.
...
func remoteDiscoveryEnabled() bool {
	value := strings.ToLower(strings.TrimSpace(os.Getenv("CORKSCREW_ENABLE_REMOTE_DISCOVERY")))
